...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public boolean saveUser(User user) {
        ...
        if (user.getIdCard() != null && baseMapper.selectCount(new QueryWrapper<User>()
                .eq("id_card", user.getIdCard())) > 0) {
    ...
    public boolean updateUser(User user) {
        ...
        if (user.getEmail() != null && baseMapper.selectCount(new QueryWrapper<User>()
                .eq("id_card", user.getIdCard()).ne("user_id", user.getUserId())) > 0) {
            throw new BusinessException("学号已存在");
        }
    ...
    @Override
    public PageResult<User> listStudentPage(PageParam<User> page) {
        List<User> users = baseMapper.listStudentPage(page);
        return new PageResult<>(users, page.getTotal());
    }
}
